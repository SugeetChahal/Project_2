...
function xScale(covidData, activeXAxis) {
    ...
    .domain([d3.min(covidData.map(d=> d[activeXAxis])), d3.max([covidData.map(d => d[activeXAxis])])])
    .range([0, chartWidth]);
...
function yScale(covidData, activeYAxis) {
    ...
    .domain(covidData.map(d => d[activeYAxis]))
    .range([chartHeight, 0])
    .padding(0.01);
...
d3.json("static/data/covid_data.json").then(function(covidData) {

    if (error) throw error;

    console.log(covidData);
    console.log(covidData.map(d => d["states"]));
    ...
    .enter()
    .append("circle")
    .attr("cx", map(d => xLinearScale(d[activeXAxis])))
    .attr("cy", map(d => yBandScale(d[activeYAxis])))
    .attr("r", 10)
