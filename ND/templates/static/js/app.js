...
function yScale(covidData, activeYAxis) {
    ...
    .domain(covidData.map(data => data[activeYAxis]))
    .range([chartHeight, 0])
    .padding(0.01);
...
function showGraph() {
    ...
    d3.csv("static/data/covid_data.csv").then(function(covidData) {
    
        // if (error) throw error;
    
        console.log(covidData);
        console.log(covidData.map(data => data.states));
        ...
        .attr("cy", (d, i) => yBandScale(d[activeYAxis][i]))
        .attr("r", 10)
